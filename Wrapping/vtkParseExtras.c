  ...
  if (m)
  {
    ...
    for (j = 0; j < m; j++)
    {
      ...
      if (cp)
      {
        ...
        if (*cp == '\0')
        {
          dim = (unsigned long)strtol(valinfo->Dimensions[j], NULL, 0);
        }
        count *= dim;
      }
    }
  ...
  ValueInfo *data, char *text, unsigned int flags)
{
  ...
  const char *tpname = data->TypeName;
  unsigned long dimensions = data->NumberOfDimensions;
  unsigned long pointer_dimensions = 0;
  size_t i = 0;
  unsigned long j = 0;
  ...
  if (func2->NumberOfParameters == func1->NumberOfParameters)
  {
    for (k = 0; k < func2->NumberOfParameters; k++)
    {
      ...
      if (!override_compatible(p2->Type, p1->Type) ||
          strcmp(p2->TypeName, p1->TypeName) != 0)
      {
        break;
      }
...
size_t vtkParse_DecomposeTemplatedType(
  ...
  unsigned long nargs, const char ***argp, const char *defaults[])
{
  size_t i, j, k, n;
  const char *arg;
  char *new_text;
